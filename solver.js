...
function SolveSudoku() {
  ...
  Search(0);
  if (!isSolved) document.getElementById("SolvedText").innerHTML = "No Solution";
...
function BuildSparseMatrix(matrix) {
  ...
  for (j = 3 * SIZE_SQUARED; j < COL_NB; j++) {
    for (var l = 0; l < SIZE_SQRT; l++) {
      for (var k = 0; k < SIZE_SQRT; k++) matrix[x + l * SIZE + k * SIZE_SQUARED][j] = 1;
    }

    var temp = j + 1 - 3 * SIZE_SQUARED;

    if (temp % (SIZE_SQRT * SIZE) === 0) x += (SIZE_SQRT - 1) * SIZE_SQUARED + (SIZE_SQRT - 1) * SIZE + 1;
...
function TransformListToCurrentGrid(Puzzle) {
  ...
  for (var n = 0; n < SIZE; n++) {
    for (var m = 0; m < SIZE; m++) {
      if (Puzzle[n][m] > 0) {
        loop1: for (var Col = HeadNode.right; Col !== HeadNode; Col = Col.right) {
